...
COPY duobleGroupBy/main.go .
COPY duobleGroupBy/go.mod .
COPY duobleGroupBy/go.sum .
#COPY ./client/src ./src
# CGO_ENABLED must be disabled to run go binary in Alpine
RUN CGO_ENABLED=0 GOOS=linux go build -o bin/duobleGroupBy


FROM busybox:latest
COPY --from=builder /build/bin/duobleGroupBy /duobleGroupBy
ENTRYPOINT ["/bin/sh"]