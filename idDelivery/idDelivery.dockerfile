...
COPY idDelivery/main.go .
COPY idDelivery/go.mod .
COPY idDelivery/go.sum .

# CGO_ENABLED must be disabled to run go binary in Alpine
RUN CGO_ENABLED=0 GOOS=linux go build -o bin/idDelivery


FROM busybox:latest
COPY --from=builder /build/bin/idDelivery /idDelivery
ENTRYPOINT ["/bin/sh"]