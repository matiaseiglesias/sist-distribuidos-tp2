...
COPY topTenPto3/main.go .
COPY topTenPto3/go.mod .
COPY topTenPto3/go.sum .

# CGO_ENABLED must be disabled to run go binary in Alpine
RUN CGO_ENABLED=0 GOOS=linux go build -o bin/topTenPto3


FROM busybox:latest
COPY --from=builder /build/bin/topTenPto3 /topTenPto3
ENTRYPOINT ["/bin/sh"]